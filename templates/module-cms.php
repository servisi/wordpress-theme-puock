<?php if (in_array(publicus_get_option('index_mode', ''), array('cms', 'company'))
    && (publicus_is_checked('cms_show_2box') || publicus_is_checked('company_show_2box'))): ?>
    <?php dynamic_sidebar('index_cms_layout_top') ?>
    <div class="row row-cols-1 <?php publicus_open_box_animated('animated fadeInUp') ?> " id="magazines">
        <?php
        $cms_mode = publicus_get_option('index_mode', '') == 'cms' ? 'cms' : 'company';
        $cms_cats_str = publicus_get_option($cms_mode . '_show_2box_id', '');
        if (!empty($cms_cats_str)) {
            $cms_cats = is_array($cms_cats_str) ? $cms_cats_str : explode(",", $cms_cats_str);
            if (count($cms_cats) > 0) {
                $cms_cats_num = publicus_get_option($cms_mode . '_show_2box_num', '6');
                foreach ($cms_cats as $catId):
                    $cache_key = sprintf(PKC_CMS_2BOX_POSTS, $catId);
                    $posts = publicus_cache_get($cache_key);
                    if (!$posts) {
                        ...
                        wp_reset_query();
                        publicus_cache_set($cache_key, $posts);
                    }
                    $post_index = 0;
                    ?>
                            ...
                            <?php foreach ($posts as $post) : setup_postdata($post); ?>
                                <?php if ($post_index == 0): ?>
                                    <div class="mtb10 magazine-media-item">
                                        <a class="img ww" <?php publicus_link_target() ?> href="<?php the_permalink() ?>">
                                            <img title="<?php the_title() ?>"
                                                 alt="<?php the_title() ?>" <?php echo publicus_get_lazy_img_info(get_post_images(), 'w-100 round-3', 120, 80) ?>/>
                                        </a>
                                        <div class="t-line-1">
                                            <h2 class="t-lg t-line-1"><a class="a-link"
                                                                         title="<?php the_title() ?>" <?php publicus_link_target() ?>
                                    ...
                                    <div class="media-link media-row-2">
                                        <div class="t-lg t-line-1 row">
                                            ...
                                            <div class="col-lg-3 text-end d-none d-lg-block">
                                                <span class="c-sub t-sm"><?php publicus_get_post_date() ?></span>
                                            </div>
                                        </div>
                                    </div>
                                <?php endif;
